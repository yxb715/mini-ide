...
**定位**：AI 时代的开发指挥台 —— 本机 GUI，替代 IntelliJ IDEA / WebStorm / PyCharm 的"启动 + 看日志 + 看文件"场景，内存 150-250 MB。用户不写代码，代码由 Claude Code 写；mini-ide 负责 **启动项目 / 看日志 / 改 markdown 文档 / Git 状态可视化**。mini-ide 与外部 Claude Code 之间不主动通信——用户的 Claude Code 在自己的终端里跑，mini-ide 不暴露 MCP / HTTP API。
...
3. 在 Alacritty 打开（可直接 claude）— `subprocess.Popen alacritty --working-directory <dir>`
...
8. **不要**重新加上"发到 AI / Claude Code"按钮：用户已明确放弃这个方向。AI 协作走外部 Claude Code（用户自己在终端开），mini-ide 不主动推送也不暴露 MCP / HTTP 接口。日志栏「📋 复制错误给 AI」是剪贴板操作，不算"推送"
